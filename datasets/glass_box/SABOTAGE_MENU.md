...
Eleven one-cell edits to the clean submission. Each names the file, the row, the
...
| 2 | `orders.csv` · ORD-01 · `due_date` → `2026-01-01` | `ids.order_dates_internally_consistent` | degraded / WARNING (`TEMPORAL_IMPOSSIBILITY`) | **CONDITIONAL** | An order is due before it was created — no schedulable window. Fix the §5.1 dates at the source. |
| 3 | `routing_lines.csv` · `RT-BRACKET,10,PRESS-SLOW` · `splittable` → `true` | `ids.alternative_step_attributes_agree` | degraded / WARNING (`AMBIGUOUS_SOURCE`) | **CONDITIONAL** | The two rows of one alternative group disagree on a *step* attribute (splittable). Resolved **first-row-wins**, disclosed. |
...
| 5 | `orders.csv` · duplicate the whole `ORD-05` row | `ids.order_identities_unique` | degraded / WARNING (`DUPLICATE_IDENTITY`) | **CONDITIONAL** | Two rows share `order_id` ORD-05; first occurrence wins, the duplicate is disclosed (proceeds flagged → WARNING). |
...
| 7 | `routing_lines.csv` · both `RT-BRACKET,10` rows · `active` → `0` | `ids.routes_resolve_to_lines` | degraded / ERROR (`ORPHAN_ENTITY`) | **CONDITIONAL** | A route step has zero active rows — an unroutable operation; the affected orders are excluded from planning. |
| 7b | `orders.csv` · ORD-09 · `quantity` → `-60` | `ids.order_quantities_are_positive` | degraded / ERROR (`VALUE_OUT_OF_RANGE`) | **CONDITIONAL** | An order quantity is negative — you cannot make -60 units. An invalid demand; the order is excluded from planning (never a floored 1-minute op). |
| 8 | `orders.csv` · ORD-01 · `quantity` → `55` **(CONTROL)** | *(none)* | — | ACCEPTED | Nothing. A legal quantity change must not raise a single flag. |
| 9 | `orders.csv` · ORD-02 · `facility_id` → `F999` | `ids.facility_references_consistent` | degraded / ERROR (`ORPHAN_ENTITY`) | **CONDITIONAL** | An order names a facility outside the declared scope; excluded. |
| 10 | `routings.csv` · RT-WIDGET · `status` → `inactive` | `ids.orders_use_active_routes` | degraded / WARNING (`LOW_CONFIDENCE_INPUT`) | **CONDITIONAL** | A live order is built on a route the ERP marks inactive — a low-confidence input (proceeds flagged → WARNING). |

## Two things worth noticing

- **Grade is a pure function of outcomes.** Any `violated` → REJECTED (item 6);
  any `degraded` → CONDITIONAL (items 1,2,3,5,7,7b,9,10); only `flagged`/`satisfied`
  → ACCEPTED (items 4, 8). The GRADE follows the *outcome*.
- **Severity follows the *disposition*, not the outcome (Session 4.5).** What the
  finding claims must match what the system did: `blocked` → BLOCKER, `excluded`
  → ERROR, `proceeded_flagged` → WARNING, quality flags → INFO. So a `degraded`
  rule can be an ERROR *or* a WARNING depending on whether the offending row is
  excluded (items 1, 7, 7b, 9 — excluded → ERROR) or proceeds flagged (items 2,
  3, 5, 10 — proceeds → WARNING). An `error` that "proceeded" was the disease
  this closed: a severity may never claim a consequence the disposition did not
  deliver.
