...
| **M5** | **Solver Builder** | Canonical snapshot + WorkPackages → CP-SAT model + **variable↔entity mapping table**. Reads plain values only, never the sidecar. Emits Decisions about its own simplifications. Consumes exactly six inputs: WorkPackages (with Operations and PrecedenceEdges), Resources, Pools, Calendars (flattened), Constraints, CostModel. |
