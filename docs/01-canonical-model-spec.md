...
| `coarse_bucket_days` | integer \| null | **Live** (R-SC2 coarse-zone amendment, docs/06 §5.9): the far-horizon look-ahead's declared bucket length in days. **`null` means the submission was SILENT**, which the coarse zone reads as the stated default (7) under `defaulted` provenance — a numeric default here would erase the distinction between the plant's choice and ours |
| `coarse_capacity_derate` | number \| null, (0,1] | **Live** (R-SC2 coarse-zone amendment, docs/06 §5.9): **rho** — the fraction of calendar capacity the coarse PLANNING run may use. `null` ⇒ the stated default **1.0, a no-op derate** under `defaulted` provenance: an undeclared plant is never given an invented safety margin. Read ONLY by the far-horizon look-ahead: **coarse never constrains fine** (amendment clause 4), so the window solve must not see it. Only a run at rho = 1.0 may be cited as a proof of infeasibility (clause 2) |
