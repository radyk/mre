...
| B2 | Eligibility / alternative resources | ✔core S | ResourceRequirement capability/explicit_set | routing_lines.resource_id → explicit_set (multiple rows per (route,sequence) = the eligible set, §5.3) + AMBIGUOUS_SOURCE when unclear | gauntlet fixed lanes · **`multi_route` scenario** (adapter grouping + solver disjunction + nonzero-price ghosts + pool cross-machine counterfactual, `tests/test_multi_route.py`) · **`multi_route_distinct`** (distinct-rate case: pool converges, the **forced-alternative service** prices the roads not taken — `add_forced_alternative_cut`, `tests/test_forced_alternatives.py`, R-T1a) · client-side **Tier-0** eligible-row dim (`src/cockpit/legality/tier0.js`) · NO_CAPABLE_RESOURCE | PP | §5.3/§5.5 |
