...
**Document 7** · Status: v3.00 · Companions: 01–04 (constitution), 05 (Constraint Catalog, in progress), 06 (Incoming Data Spec)

**v3.00:** **Session S-02 + S-03 — the certificate contract.** 2026-08-05 (**R-CT1 ruled and BUILT**, verbatim in docs/04; **`CONTRACT_VERSION` UNCHANGED at 1.15** and the brief's expectation of a bump was wrong on a rule docs/02 §4.2 already states — that constant versions the SCHEDULE DOCUMENT and no field added here reaches it, so the governing rule is add-never-repurpose with the spec update in the same commit; **docs/02 AMENDED** at §4.4, §4.5 and §6; no docs/06 doorway owed; **BOTH GOVERNED PROMPTS UNCHANGED** — parse **v19**, synthesis **v9**, every seam here being gate code, contract text, authored copy and tests; narrative in `docs/closeouts/4x-certificate-contract.md`). Closes ledger rows **S-02** and **S-03** from the shared-body census. **THE PREDICATE-AUDIT INSTANCE RESOLVED §5a.222(c) AND THE CAUSE IS AN (e2) REPORTING ERROR** — a 2×2 of instrument × corpus (claim_verifier at 3f44004 / 50dfccb / 6bf0ed9 × the 50-transcript and 54-transcript corpora) reads **5 non-GK firings in every cell**, so it is neither instrument change, nor corpus drift, nor invocation. The 2 is `f1_widening_trial.py`'s **`(context)` line**, which counts **pattern (a) ALONE** (`_PRODUCT_BEHAVIOR_RES[0][1]`) — re-run today it still prints `shipped 2 -> candidate 2`, and exactly 2 of the 5 whole-predicate firings carry pattern (a)'s reason. It was transcribed into a table whose other two rows came from the whole-predicate ruler. **THE OPERATIVE PRECISION FIGURE IS 5** (of 401 on (e2)'s corpus, of 404 today), and **none of the 5 is a live false positive**: all five are `synthesis — read from:` CITED lines, and the predicate is gated on `not cited`. **S-02 — AN ACCEPTED SUBMISSION LEFT THE EVIDENCE STORE SILENT ABOUT ITS OWN CERTIFICATE.** `record()` emits a Finding only for a rule that is NOT satisfied, which is correct and stays correct (every finding code names a defect), so the grade was computed, written to `certificate.json`, and never reported — every surface reading evidence alone was **structurally** unable to state it. The verdict now enters the store **at both of the gate's exits** (the cascade and the intake refusal — “I was pointed at nothing” is still a grade), decomposed across three EXISTING record types: the categorical part on a §4.5 **Event** (`gate_verdict`, tier headline, subjects naming the submission), coverage on §4.4 **Metrics** (`gate.rules_checked` rolling up the four outcome counts, **emitted even at zero** so the decomposition is verifiable rather than asserted), and `certificate.json` on a §4.6 **Artifact** with its sha256. **FOUR KINDS WERE REFUSED WITH REASONS**: a Finding (an ACCEPTED grade would wear a defect's code), a Decision (the grade is a PURE FUNCTION, not a choice — no alternatives to enumerate and `driver` is mandatory-exactly-one where every code names a *scheduling* cause), a Metric for the grade itself (`value` is a float, a grade is a word), and a new record type. **Provenance is `derived` with a WALKABLE formula id** — the guard asserts it resolves to a callable — and is a use of the docs/01 §7 vocabulary, **not** a sidecar write, M0 running before canonical identities exist. **THE READ HAS ONE DEFINITION AND ONE STATED ORDER, evidence-first / artifact-second**, with `source` naming which reading answered; **the fallback is RETAINED and is not a legacy path** — evidence is append-only, so every board gated before this change has no record and never will. **NO RETROACTIVE WRITES.** `record_event` grew an optional `subjects` parameter (the Event model has carried the field since L1; the verb hardcoded `[]` — a REPORTER limitation, never a contract one), additive, every existing call site byte-identical. **S-03 — THE SIGNING SENTENCE IS GONE AND COUNTERSIGNING IS PARKED, NOT SCAFFOLDED** (arbitrated by Daryn: removal, not build). *“…and it is unsigned — nobody has countersigned it”* was true of the artifact and false as an implication: it tells a planner a signing step exists and was skipped when the product has no such concept — **the manufacture rule's quieter cousin, asserting the absence of something implies the something**. What replaces it is provenance: whose record it is, when it was made, and that *the grade is computed from those rule outcomes* — checkable, and stated ONLY where the reading carries provenance (an artifact read has none to quote and does not quote one). **The copy was CENSUSED, not spot-fixed**: one live site; every other repo hit is historical record, the arithmetic sense of *sign*, or R-CAL1's profile signature. **R-CAL1 IS UNTOUCHED AND A GUARD SAYS SO**, so no future session reads S-03 as precedent against it. **A DEFECT THE GUARD CAUGHT AND A CODE READ WOULD NOT HAVE**: the first artifact hash was taken from the JSON string, and `write_text` newline-translates on Windows — the digest did not verify against the file it named. **Python 2903/305/0 UNCHUNKED in 18m29s** against a baseline of **2882/305/0** ((d.3)'s measured after-run at HEAD `6bf0ed9`) — **+21 exactly**, the new guard file collects exactly 21, confirmed independently by collection **3208 vs 3187** (3187 being byte-for-byte (d.3)'s recorded number). **NO RESIDUAL.** **6 negative controls proven RED**, each GREEN AT HEAD first, every restore byte-identical by sha256 — **and one stayed GREEN on its first run and it was the CONTROL's fault, not the seam's**: its anchor `subjects=subjects or [],` matches `record_metric` FIRST, so it reverted the wrong verb entirely; re-anchored on a line unique to `record_event` it goes red. (d.2)'s lesson at a third site. **MINTED NOTHING in `_data`; both pinned worlds read-only; cockpit untouched and not re-run.** NOT FIXED, named: the board opener still returns `grade: None` though the record it needs now exists; the verdict Event is reachable by key only on a schema-2 index; `certificate.md` is written but not registered. §5a.223-226.
...
**PARKED CAPABILITY — certificate countersigning** (2026-08-05, R-CT1(2); arbitrated: parked, not built, not scaffolded, not implied in copy anywhere). Two prerequisites, verbatim: **(1) define what the signer is attesting to; (2) identify when customers actually require human approval.** Neither is answered, so there is no design here and none is owed. R-CAL1's `CalibrationProfile` signature is a DIFFERENT artifact with a defined attestation and is untouched by this.
...
0. **THE ROLLING INTERACTION STACK — DIAGNOSED 2026-08-07 (Session R4.0 recon,
   ...
   * **R4.2 — what a child inherits at accept (D3, D4). ✅ DONE 2026-08-08 —
     R-CH1 ruled and BUILT** (§5a.258-264; docs/04 2026-08-08;
     `docs/closeouts/r4-2-child-inheritance.md`). Both accept ceremonies mint a
     child of the parent's own KIND, through one inheritance seam
     (`schedule_assembler.inherit_child_metadata`); the reference date and the
     M5 horizon are RECORDED at both ceremonies so `derive_base_context` and
     `_m5_horizon` recover them from a child's own run dir. Proven end to end on
     a scratch copy of gen-3: **the child's beat-one table is identical, probe
     for probe, to the correctly scoped parent's — 32 probes, 22 possible, 10
     impossible, 0 false refusals, 0 false permissions, 0 frame errors** against
     `b5daba66`'s 24 of 24 impossible. **The what-if is NOT fixed and turns out
     to be broken one seam earlier** (§5a.263) — it fails on every rolling board
     at the snapshot copy, so its fix is a session, not a line. **Lineage replay
     onto gen-3 is now unblocked** and was deliberately not run here. Original
     brief text follows.
     Opens `app.py`, `schedule_assembler.py`, `scenario.py`. Unblocks the
     lineage (S2, S3) and gen-3's deferred child. Needs R4.1's frame assertion in place to PROVE the
...
1. **Rolling-child species — ✅ CLOSED 2026-08-08 by R4.2 (R-CH1, §5a.258-264).**
   An accept on a rolling board minted a monolithic, uncalibrated child; it now
   mints a child of its parent's own kind. The design question *what survives a
   re-solve* is answered in R-CH1's four clauses.
1a. **THE WHAT-IF ON A ROLLING BOARD — MEASURED BROKEN, NOT FIXED (R4.2 W6,
   §5a.263).** Two independent defects, in this order: (i) `_execute_whatif`
   copies `base_run["snapshot_id"]` where the accept deliberately uses
   `base_schedule["snapshot_id"]`, so on every rolling board it fails with
   `FileNotFoundError` (`snap-<run[:8]>` against `snap-rolling`) — the accept was
   corrected for this in 4.0d and the what-if never was; (ii) even repaired, a
   scenario is a full pipeline re-run over its own horizon and does NOT place the
   parent's plan of record, so R-CH1 clause (1)'s frame check correctly refuses
   to graft the parent's window onto it. A rolling what-if needs rolling
   RE-DERIVATION, which is the same shape as the *pool service must become
   slice-aware* debt and belongs with it. **A session, not a line.**
...
**§5a.223 — THE CENSUS RECONCILIATION: §5a.222(c) RESOLVED, CAUSE NAMED.** The
standing ruler read **5** non-GK firings where (e2) recorded **2** over the same
522-line corpus. Settled by a 2×2 (instrument × corpus) plus the before-F1 instrument:
**every cell reads 5** — claim_verifier at (e) `3f44004`, (e2) `50dfccb` and (d.3)
`6bf0ed9`, over both the 50-transcript and the 54-transcript corpora. So it is **not**
an instrument change, **not** corpus drift, and **not** the invocation. The cause is an
**(e2) reporting error**, identifiable to the line:
`tools/spikes/teaching_graft_e2/f1_widening_trial.py:90-92` counts
`_PRODUCT_BEHAVIOR_RES[0][1]` — **pattern (a) alone** — and prints it under its own
`(context)` label. Run today it still prints `shipped 2 -> candidate 2`, and exactly
**2 of the 5** whole-predicate firings carry pattern (a)'s reason. That one-pattern
number was transcribed into a table whose other two rows are the whole-predicate
ruler's. **The operative figure is 5** — of 401 on (e2)'s corpus, of 404 today — and
**none of the 5 is a live false positive**: all five are `synthesis — read from:`
CITED lines, and `product_behavior_disqualifiers` is consulted only under `not cited`.
The instrument is not at fault and needs no change. **The lesson is the
transcription:** a script's `(context)` line is not the measurement, and two
instruments' outputs in one table under a shared heading is how a number stops meaning
what it says.

**§5a.224 — S-02: THE GATE'S VERDICT IS EVIDENCE (R-CT1(1)).** Full reasoning in
docs/04; the record shape and the four refusals are there and are not restated here.
What belongs in the roadmap: the gap was **structural, not cosmetic** — an ACCEPTED
submission was silent in the store, so *every* evidence-only surface was disabled, and
the certificate route's artifact read was a workaround for a contract gap rather than a
preference. **The pinned-worlds line:** `gb_pinned` (gated 2026-07-26) and every board
minted before this commit carry **no** verdict record and never will — evidence is
append-only and nothing was written retroactively. They answer from the artifact, and a
guard asserts exactly that on the real board. Boards minted from this commit answer
from the record, cite its `record_id`, and state the grade's provenance.

**§5a.225 — S-03: THE SIGNING SENTENCE IS REMOVED; COUNTERSIGNING IS PARKED
(R-CT1(2)).** The capability entry, with the two prerequisites verbatim and no design,
is in §4 (Post-pilot sequence) as a PARKED CAPABILITY. **R-CAL1 is untouched and
undiminished**, and a guard in this session's file asserts rule (2) still holds so that
S-03 cannot be read as precedent against it. The copy census found **one** live site;
the other repo hits are historical record (docs/04, docs/07, the census close-out,
committed sweep transcripts — all correctly left alone), the arithmetic sense of
*sign*, and the profile signature.

**§5a.226 — THIS SESSION'S CARRY-FORWARDS (REPORTED, deliberately NOT fixed).**
**(a) THE BOARD OPENER STILL RETURNS `grade: None`.**
`Explainer._opener_certificate` was the named specimen of the S-02 gap (the 4B.16 debt,
§5a.49); the record it needs now exists and it does not yet read it. Left alone
deliberately — the brief scoped the certificate ROUTE, and the opener is a different
surface with its own owner. It is now a one-call fix rather than a contract change.
**(b) THE VERDICT EVENT IS SUBJECT-REACHABLE ONLY ON A SCHEMA-2 INDEX.** A schema-1
index dropped every subject-less record, and `_V1_LOST_CLASSES` still names `event` for
that reason — the class list stays true of those files. New Events carry subjects, so
the loss is historical. Stated as a bound, not a claim of cleanliness.
**(c) `certificate.md` IS WRITTEN BUT NOT REGISTERED.** The verdict record references
`certificate.json`, which is where the contract line is drawn; the markdown rendering of
the same certificate is an output nobody registered. One line, not taken, because
tidying an adjacent artifact is not this session's scope.
**(d) THE ARTIFACT AND EVIDENCE BODIES DIFFER BY ONE CLAUSE.** The provenance sentence
is stated only on the evidence path, because only that reading carries provenance. That
...
**§5a.258 — R-CH1 RULED AND BUILT: A CHILD IS THE SAME KIND OF THING AS ITS
PARENT (Session R4.2, 2026-08-08).** Second fix session off the R4.0 dossier,
closing D3 and D4. Both accept ceremonies now mint their child through
`schedule_assembler.inherit_child_metadata`: a rolling parent yields a ROLLING
child (window, frozen front, tray, coarse zone and boundary-move log inherited;
each bar's `commitment_state` and the two counts DERIVED from the child's own
placements against the inherited boundary), calibration is inherited by
declaration with the parent named in its sentence, and the portfolio is not
inherited at all. A monolithic parent still mints a monolithic child. Five files
changed (`schedule_assembler.py`, `api/app.py`, `planner_edit.py`, `sandbox.py`,
`scenario.py`), one test file added (`tests/test_child_inheritance.py`, 40 tests
— 26 fast, 14 slow), two probes committed
(`p9_child_inheritance.py`, `p10_ch1_controls.py`). **No contract change**
(1.17), prompts unchanged (v19 / v9), no vocabulary change, no pinned world
written to.

**§5a.259 — THE MECHANISM, AND WHY NOT THE OTHER ONE.** The obvious route was
`assemble_rolling_document`, which is what a rolling solve calls. It needs a
`RollingView`, and a `RollingView` is the WINDOW SOLVE's own record — `status`,
`gap`, `op_drivers`, `cost_ledger`, `incumbent_trail`, `earliness_tiebreak` are
all statements about a solve an accept never ran. Fabricating one would have
written the accept's re-solve telemetry into fields that mean the window solve's,
which is the 4B.23 "a default that ASSERTS" class. So the mechanism is
INHERITANCE: what the child's own run genuinely produces (placements, ledger,
service outcomes, solver telemetry) the monolithic assembler already renders
correctly, and only the LINEAGE metadata is carried across. **The inherited
window is asserted, not assumed** — the child's placed-operation set is checked
against the parent's with no tolerance, because the tray and window describe the
parent's PLAN and inheriting them is sound only while an accept re-places that
plan (R-DP11). A mismatch raises `ChildInheritanceError`.

**§5a.260 — CLAUSE (3) FOUND A CLAIM A NAIVE COPY WOULD HAVE MANUFACTURED.**
R-CAL1 gives `applied` exactly one meaning — the coefficients this solve actually
took — and an accept re-solve takes none of them: it runs at the sandbox's own
deterministic budget and seed. Copying the parent's calibration block verbatim
would therefore have stated that the child ran at the calibrated budget. So the
profile's IDENTITY is inherited and `applied` / `window_solved` / `drift` are
CLEARED, with the provenance in the block's own `sentence`. Measured on gen-3:
parent `applied` True → child False, state `accepted` on both, parent id in the
child's sentence. The provenance rides in `sentence` deliberately — an
`inherited_from` field would be a contract shape change, and the sentence is
what every calibration surface already renders.

**§5a.261 — THE THIRD MEMBER OF THE CLASS, FOUND BY CENSUSING IT.** The dossier
named `planner_edit` as D4's seam. `materialize_audit_offer` recorded neither the
reference date NOR the horizon on its M5 model build, so an audit-accept child
...
failed outright. Both fields are now recorded, and the regression exercises the
REAL write on a board whose incumbent holds (the ceremony writes its M5 record
before it decides there is nothing to accept, so its refusal is the positive
control that it genuinely ran).

**§5a.262 — W5, THE END-TO-END PROOF: THE FOUNDING SYMPTOM, DEAD.** One real
accept through `POST /schedules/{id}/accept` on a scratch copy of gen-3
(`p9_child_inheritance.py`). Child: rolling True, window 10/1, frozen front
2026-01-06, 122 tray entries, coarse zone present, committed 24 / active 362,
386 of 386 placements = the accepted plan, 0 bars moved, calibration `accepted`
with `applied` cleared, no portfolio, `reference_date` recoverable as
`2026-01-05T00:00:00+00:00`. Then beat one through the real endpoint, 8 ops × 4
offsets: **the child's table is identical, probe for probe, to the correctly
...
impossible and 23 false sentences (R4.0 §3.4).

**§5a.263 — W6: THE WHAT-IF IS BROKEN ONE SEAM EARLIER THAN THE ASSEMBLER.**
Assessed, NOT fixed (the brief walls it out). Under this mechanism the assembler
change at the what-if is one line — and it would be WRONG: a scenario is a full
pipeline re-run over its own horizon and does not place the parent's plan of
record, so clause (1)'s frame check would correctly refuse it. A what-if of a
rolling board needs rolling RE-DERIVATION, not inheritance, which is the standing
"pool service must become slice-aware" debt. **And it never reaches that
question. Measured: the what-if fails outright on every rolling board** with
`FileNotFoundError` on the snapshot copy, because `_execute_whatif` reads the
RUN's minted snapshot id (`snap-<run[:8]>`) where the accept deliberately reads
the SCHEDULE's — and a rolling solve's schedule snapshot is always
`snap-rolling`. The accept was corrected for this in 4.0d; the what-if never was.
**The what-if fix is a session, not a line.**

**§5a.264 — R4.2's CARRY-FORWARDS (reported, deliberately NOT fixed).**
**(a) THE SCHEDULE PICKER STILL TAGS AN ACCEPT CHILD OF A ROLLING BOARD AS
MONOLITHIC.** `scheduleKind` (`src/cockpit/src/schedulepicker.js`) reads the id
prefix and the snapshot id, and an accept child is `<uuid4>` on
`snap-edit-<sha12>` — neither spells "rolling". The DOCUMENT is now correct; the
LISTING row is not, and the picker's own comment already names the durable fix
(a registry `sliced` column, a schema change).
**(b) THE ABSENT-PORTFOLIO STATE IS SILENCE, NOT A SENTENCE.** Verified rather
than rebuilt, per the brief: `summary.js` appends the portfolio box only
`if (portfolio)`, so a child simply renders no portfolio section. That asserts
nothing and is honest, but it is NOT the explicit three-state treatment
`progressModel` gives the solve-progress trail. Named at the level it was
verified at.
**(c) THE `applied`-CLEARED ASSERTION IS VACUOUS ON THE TEST FIXTURE.** The test
data root holds no measured profile, so the parent's own `applied` is already
`{}` and the child's being empty proves nothing there. The clause is exercised by
the fast unit tests and, at demo density, by the gen-3 measurement in §5a.260 —
said in the test's own docstring rather than left to be assumed.
**(d) LEGACY CHILDREN ARE STILL DATELESS AND STILL LOUD.** `b5daba66` and every
child minted before this ruling remain unrepaired by design: R-SG1's frame
invariant refuses them rather than answering from a mis-framed model. Confirmed
unchanged at HEAD by `p6_frame_catches` and NOT repaired here.
**(e) THE AUDIT-ACCEPT CEREMONY'S CHILD DOCUMENT IS UNPROVEN END-TO-END.** The
fixture's incumbent is already optimal, so the deeper search offers nothing and
`/audit/accept` cannot be driven at fixture density. What is proven is the M5
WRITE (§5a.261) and the inheritance function both ceremonies share; the audit
ceremony's own child document is asserted by construction, not measured.
